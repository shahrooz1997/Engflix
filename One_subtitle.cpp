...
One_subtitle::One_subtitle(const string &in, Subtitle *parent) : parent(parent) {
...
std::chrono::time_point<std::chrono::steady_clock,
                        std::chrono::milliseconds> One_subtitle::stotime(const std::string &in) {
...
const One_subtitle *One_subtitle::get_next() const {
  if (get_id() < parent->get_subtitles().size()) {
    return &parent->get_subtitles()[get_id()];
  }
  return nullptr;
}

const One_subtitle *One_subtitle::get_prev() const {
  if (get_id() - 2 > 0) {
    return &parent->get_subtitles()[get_id() - 2];
  }
  return nullptr;
}
